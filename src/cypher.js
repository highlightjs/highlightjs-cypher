...
module.exports = function (hljs)
{
  return {
    ...
    keywords:
      {
        keyword: 'as asc ascending and assert by call case collect commit constraint create csv cypher delete desc descending detach distinct drop else end ends exists explain fieldterminator finish for foreach from headers in index is join limit load match merge nodetach not on optional order periodic profile remove return scan set skip start starts then union unique unwind use using when where with yield',
